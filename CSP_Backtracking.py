...
"""
    ...
    # Solve Sudoku problem using CSP - Backtracking Algorithm
"""
...
def backtrack(assignment, csp):
    ...
    for value in order_domain_values(var, assignment, csp):
        if csp.nconflicts(var, value, assignment) == 0:
            ...
            removals = [(var, v) for v in csp.curr_domains[var] if v != value]
...
def select_unassigned_variable(assignment, csp):
    return minimum_remaining_values(assignment, csp)

def order_domain_values(var, assignment, csp):
    return least_constraining_value(var, assignment, csp)

def inference(csp, var, value, assignment, removals):
    return forward_checking(csp, var, value, assignment, removals)

def restore(csp, removals):
    for B, b in removals:
        if b not in csp.curr_domains[B]:
            csp.curr_domains[B].append(b)


if __name__ == '__main__':
    init_assign_hard = '530070000600195000098000060800060003400803001700020006060000280000419005000080079'
