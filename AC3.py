"""
    ...
    # Solve Sudoku problem using CSP -  AC3 Algorithm
"""
import itertools
import re
import random
...
def AC3(csp):
    ...
    csp.curr_domains = csp.domains.copy()  # Make a copy for inferences

    while queue:
        (Xi, Xj) = queue.pop()
        if revise(csp, Xi, Xj):  # If a revision is made
            if not csp.domains[Xi]:  # If domain of Xi is empty, return failure
                return False
            # Add all neighbors of Xi except Xj back into the queue
            for Xk in csp.neighbors[Xi] - {Xj}:
                queue.add((Xk, Xi))
    return True  # CSP is satisfiable if no conflicts remain
...
if __name__ == '__main__':
    init_assign_easy = '530070000600195000098000060800060003400803001700020006060000280000419005000080079'
